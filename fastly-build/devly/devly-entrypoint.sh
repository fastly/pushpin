...
if [ ! -f "$PUSHPIN_BIN" ] || [ "$(find src Cargo.toml Cargo.lock -newer "$PUSHPIN_BIN" 2>/dev/null | head -1)" ]; then
    echo "Building pushpin..."
    cargo fetch
    make RELEASE=1 PREFIX=/opt/fst-pushpin CARGO_ARGS="--features no-seccomp"
