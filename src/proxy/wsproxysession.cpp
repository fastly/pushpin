...
public:
        ...
        if (target.type == DomainMap::Target::Test) {
            ...
            if (target.overHttp) {
                ...
                aboutToSendRequestConnection = woh->aboutToSendRequest.connect(
                    boost::bind(&Private::out_aboutToSendRequest, this));
    ...
    void incCounter(Stats::Counter c, int count = 1) {
        if (statsManager)
            statsManager->incCounter(route.statsRoute(), c, count,
                                     inSock ? inSock->requestUri().host() : QString());
    }
    ...
    void out_connected() {
        ...
        QList<QByteArray> wsExtensions =
            headers.takeAll("Sec-WebSocket-Extensions").asQByteArrayList();
    ...
    void out_aboutToSendRequest() { updateGripSig(); }
