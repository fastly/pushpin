...
void manipulateRequestHeaders(const char *logprefix, void *object, HttpRequestData *requestData,
    ...
    if (!cdnLoop.isEmpty()) {
        QList<QByteArray> values =
            requestData->headers.takeAll("CDN-Loop", true).asQByteArrayList();
        values += cdnLoop;
    ...
    if (!trustedClient && gripEnabled) {
        ...
        requestData->headers +=
            HttpHeader("Grip-Feature",
                       ...
                       "experimental:filter:http-check, experimental:filter:http-modify");
    ...
    QList<QByteArray> xffValues =
        requestData->headers.takeAll("X-Forwarded-For").asQByteArrayList();
    if (xr->truncate >= 0)
