...
public:
    ...
    RunnerArgsData() : mergeOutput(false), id(-1) {}
};

static CommandLineParseResult parseCommandLine(QCommandLineParser *parser, RunnerArgsData *args,
    ...
    void start() {
        ...
        if (serviceNames.contains("proxy"))
            services += new PushpinProxyService(
                ...
                filePrefix, logLevels.value("proxy", defaultLevel), args.routeLines);
