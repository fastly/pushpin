...
static QList<PublishItem> parseItems(const VariantList &vitems, bool *ok = 0,
...
static QByteArray decryptMessage(const QByteArray &data, const QHash<QString, QByteArray> &keys,
    ...
    if (plain.isNull()) {
        switch (e) {
        case Encrypt::InvalidInput:
            *errorMessage = "invalid input";
            break;
        case Encrypt::UnsupportedAlgorithm:
            *errorMessage = "unsupported algorithm";
            break;
        case Encrypt::BadFormat:
            *errorMessage = "bad format";
            break;
        case Encrypt::InvalidData:
    ...
    InspectWorker(ZrpcRequest *_req, ZrpcManager *_stateClient, bool _shareAll)
        ...
        if (req->method() == "inspect") {
            ...
            for (const Variant &vheader : args["headers"].toList()) {
                ...
                if (vlist.count() != 2 || typeId(vlist[0]) != VariantType::ByteArray ||
                    typeId(vlist[1]) != VariantType::ByteArray) {
                    respondError("bad-request");
                    return;
                }
    ...
    void doFinish() {
        ...
        if (autoShare && requestData.method == "GET") {
            ...
            QList<QByteArray> gripLastHeaders =
                requestData.headers.getAll("Grip-Last").asQByteArrayList();
...
public:
    ...
    void start() {
        ...
        if (args.contains("inspect")) {
            ...
            if (!vinspect.contains("no-proxy") ||
                typeId(vinspect["no-proxy"]) != VariantType::Bool) {
                respondError("bad-request");
                return;
            }
...
private:
    ...
    void sequencer_itemReady(const PublishItem &item) {
        ...
        if (!responseSessions.isEmpty()) {
            ...
            QList<QByteArray> exposeHeaders =
                f.headers.getAll("Grip-Expose-Headers").asQByteArrayList();
...
private:
    ...
    Variant parseJsonOrTnetstring(const CowByteArray &message, bool *ok = 0,
                                  QString *errorMessage = 0) {
        Variant data;
        bool ok_;
    ...
    void inSub_readyRead(const CowByteArrayList &message) {
        ...
        bool ok;
        QString errorMessage;
        Variant data = parseJsonOrTnetstring(payload, &ok, &errorMessage);
        if (!ok) {
