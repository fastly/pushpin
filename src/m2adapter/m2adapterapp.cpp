...
public:
        ...
        if (zresp.type == ZhttpResponsePacket::Data) {
            ...
            if (s->mode == Http) {
                ...
                if (!zresp.body.isEmpty() || firstDataPacket) {
                    if (firstDataPacket) {
                        ...
                        QList<QByteArray> connHeaders =
                            headers.takeAll("Connection").asQByteArrayList();
                        ...
                        m2_queueHeaders(
                            s->conn,
                            createResponseHeader(zresp.code, zresp.reason.asQByteArray(), headers));
                    }
            ...
            } else // WebSocket
            {
                if (!s->sentResponseHeader) {
                    ...
                    QByteArray frame = makeWsHeader(!zresp.more, opcode, zresp.body.size()) +
                                       zresp.body.asQByteArray();
            ...
            if (s->mode == WebSocket && zresp.condition == "rejected") {
                ...
                m2_queueHeaders(s->conn, createResponseHeader(
                                             zresp.code, zresp.reason.asQByteArray(), headers));
    ...
    void m2_in_readyRead(const CowByteArrayList &message) {
        ...
        if (!s) {
            ...
            Url uri = Url::fromEncoded(uriRaw, Url::StrictMode);
            if (!uri.isValid()) {
    ...
    void zhttp_in_readyRead(const CowByteArrayList &message) { handleZhttpIn(Http, message); }

    void zws_in_readyRead(const CowByteArrayList &message) { handleZhttpIn(WebSocket, message); }
