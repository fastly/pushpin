...
public:
		...
		reportTimer(0)
	{
		...
		prometheusMetrics += PrometheusMetric(PrometheusMetric::WSError,"ws_error", "counter", "Number of websocket errors");
	...
	void prometheus_requestReady()
	{
		...
		foreach(const PrometheusMetric &m, prometheusMetrics)
		{
			...
			switch(m.mtype)
			{
				...
				case PrometheusMetric::WSError: value = QVariant(combinedReport.messagesSent); break;
			}
