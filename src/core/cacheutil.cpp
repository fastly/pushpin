...
void store_cache_response_buffer(const QByteArray& itemId, const QByteArray& responseBuf, QString msgId, int addLen)
{
	...
	log_debug("[STORE_BUFF]-%s %s", itemId.constData(), buff.constData());
...
QByteArray load_cache_response_buffer(const QByteArray& instanceAddress, const QByteArray& itemId, QByteArray packetId, int seqNum, QString msgId, QByteArray from, int addLen=0)
{
	...
	log_debug("[LOAD_BUFF]-%s %s", itemId.constData(), buff.constData());
...
int parse_packet_msg(Scheme scheme, const ZhttpRequestPacket& packet, PacketMsg& packetMsg, const QByteArray& instanceId)
{
	...
	else
	{
		...
		//if (is_subscribe_method(packetMsg.method) == true)
		//{
		//	subKey += instanceId.constData();
		//}
		packetMsg.paramsHash = build_hash_key(jsonMap, subKey);
	}
...
{
	exit_process_for_cacheclient(_no);
...
{
	if (gWsCacheClientList[_no].wscatWorker != nullptr)
	{
		gWsCacheClientList[_no].wscatWorker->stopWscat();
		gWsCacheClientList[_no].wscatWorker = nullptr;
	}
	if (gWsCacheClientList[_no].wscatThread != nullptr)
	{
		gWsCacheClientList[_no].wscatThread->quit();
		gWsCacheClientList[_no].wscatThread->wait();
		gWsCacheClientList[_no].wscatThread = nullptr;
	}
}
