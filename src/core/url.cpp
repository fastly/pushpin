...
Url::Url(const QString &url, ParsingMode mode) {
    QByteArray utf8 = url.toUtf8();
    handle_ = ffi::url_from_encoded(reinterpret_cast<const uint8_t *>(utf8.constData()),
                                    utf8.size(), static_cast<ffi::UrlParsingMode>(mode));
}

Url::Url(const char *url) { handle_ = ffi::url_from_string(url); }

Url::Url(const Url &other) { handle_ = ffi::url_clone(other.handle_); }
...
Url Url::fromUserInput(const QString &userInput) {
    // Simple implementation: try to parse as-is first, then with http:// prefix if it fails
    Url url(userInput);
    if (url.isValid()) {
        return url;
    }

    // If it doesn't start with a scheme, try adding http://
    if (!userInput.contains("://")) {
        return Url("http://" + userInput);
    }

    return url; // Return the invalid URL
}

QString Url::fromPercentEncoding(const QByteArray &input) {
    // Use Qt's percent decoding for compatibility
    return QString::fromUtf8(QByteArray::fromPercentEncoding(input));
}

QByteArray Url::toPercentEncoding(const QString &input) {
    // Use Qt's percent encoding for compatibility
...
bool Url::isEmpty() const { return ffi::url_is_empty(handle_); }

bool Url::isValid() const { return ffi::url_is_valid(handle_); }

void Url::clear() { ffi::url_clear(handle_); }
...
QString Url::path(ComponentFormattingOptions options) const {
...
QString Url::query(ComponentFormattingOptions options) const {
...
bool Url::hasQuery() const { return ffi::url_has_query(handle_); }

QString Url::toString(ComponentFormattingOptions options) const {
...
int Url::port() const { return ffi::url_port(handle_); }

int Url::port(int defaultPort) const {
    int p = ffi::url_port(handle_);
    return (p == -1) ? defaultPort : p;
}
...
void Url::setPort(int port) { ffi::url_set_port(handle_, port); }

void Url::setPath(const QString &path, ParsingMode mode) {
    Q_UNUSED(mode); // Rust url crate doesn't have parsing modes for paths
...
void Url::setQuery(const UrlQuery &query) { setQuery(query.toString()); }
...
bool Url::operator!=(const Url &other) const { return !(*this == other); }
...
Url Url::resolved(const Url &relative) const { return resolved(relative.toString()); }
