...
impl<T> Backend for Slab<SlabNode<T>> {
    ...
    type Index = usize;
    type Borrow<'a>
        = &'a SlabNode<Self::Value>
    where
        Self::Value: 'a;
    type BorrowMut<'a>
        = &'a mut SlabNode<Self::Value>
    where
        Self::Value: 'a;
...
pub struct NodeDataInner<T> {
    pub prev: Option<RcNode<T>>,
    pub next: Option<RcNode<T>>,
    pub value: T,
}

pub struct NodeData<T>(RefCell<NodeDataInner<T>>);
...
impl<T> RcNode<T> {
    pub fn new(value: T, memory: Option<&Rc<NodeMemory<T>>>) -> Self {
        let data = NodeData(RefCell::new(NodeDataInner {
            prev: None,
            next: None,
            value,
        }));
    ...
    pub fn borrow(&self) -> Ref<'_, NodeDataInner<T>> {
        match self {
            Self::Arena(r) => r.get().0.borrow(),
            Self::Std(r) => r.0.borrow(),
        }
    }

    pub fn borrow_mut(&self) -> RefMut<'_, NodeDataInner<T>> {
        match self {
            Self::Arena(r) => r.get().0.borrow_mut(),
            Self::Std(r) => r.0.borrow_mut(),
        }
    }
...
impl<T> Backend for RcBackend<T> {
    ...
    type Borrow<'a>
        = Ref<'a, NodeDataInner<Self::Value>>
    where
        Self::Value: 'a;
    type BorrowMut<'a>
        = RefMut<'a, NodeDataInner<Self::Value>>
    where
        Self::Value: 'a;
    ...
    fn clone_link(
        ...
        let node = index.borrow();

        match rel {
            Relation::Prev => node.prev.clone(),
            Relation::Next => node.next.clone(),
        }
    }
    ...
    fn set_link(
        ...
        let mut node = index.borrow_mut();

        match rel {
            Relation::Prev => node.prev = value,
            Relation::Next => node.next = value,
        }
    }
    ...
    fn take_link(
        ...
        match rel {
            Relation::Prev => node.prev.take(),
            Relation::Next => node.next.take(),
        }
    }
