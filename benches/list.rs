...
fn criterion_benchmark(c: &mut Criterion) {
    {
        ...
        let mut nodes_slab = Some(Slab::with_capacity(1000));

        c.bench_function("index slab list push pop 1000", |b| {
            b.iter(|| {
...

    {
        ...
        c.bench_function("generic slab list push pop 1000", |b| {
            b.iter(|| {
                let mut nodes = nodes_slab.take().unwrap();
    ...
    {
        // Preallocate the nodes memory
        let nodes_memory = Rc::new(arena::RcMemory::new(1000));

        c.bench_function("arena rc list push pop 1000", |b| {
            b.iter(|| {
                ...
                let mut expected_value = 0;
                while nodes_memory.len() > 0 {
                    let n = l.pop_front(&mut b).unwrap();
                    assert_eq!(n.borrow().value, expected_value);
                    expected_value += 1;
                }
            })
        });
    ...
    {
        c.bench_function("std rc list push pop 1000", |b| {
            b.iter(|| {
                let mut b = list::RcBackend::default();
                let mut l = list::GenericList::default();
